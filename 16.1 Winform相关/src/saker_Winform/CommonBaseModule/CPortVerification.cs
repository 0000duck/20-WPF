...
{
    class CPortVerification
    {
        public static List<int> PortValid = new List<int>();
        /// <summary>
        /// 获取第一个可用的端口号
        /// </summary>
        /// <returns></returns>
        public static int GetAvailablePort(int count)
        {
            ...
            int BEGIN_PORT = 11110;//从这个端口开始检测
            count = count * 4;
            for (int i = BEGIN_PORT; i < MAX_PORT; i++)
            {
                if (PortIsAvailable(i))
                {
                    if (i == 22222) continue; // 22222状态监测端口
                    PortValid.Add(i);
                    if (PortValid.Count == count)
                    {
                       return  count;
                    }
                }              
