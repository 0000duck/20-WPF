...
#define MAX_BUFFER_SIZE 4000042
...
void create_HuffmanTree(int* wet, int n)
{
...
int HuffmanDeCoding1(unsigned char* pu8DataBuff, int u32DataLength,const char * fileout)
{
	if (HT == NULL)
	{
		return -1;
	...
	int root = 2 * nodes - 2;
	while (current_bit < u32DataLength*8)
	{		
		int bit = (pu8DataBuff[current_bit >> 3] >> (7 - current_bit % 8)) & 0x1;
		//fprintf(fout, "%d", bit);
		if (bit == 0)
		{
			root = HT[root].lchild;
