...
namespace saker_Winform.CommonBaseModule
{
    public class CXmlHelper
    {
        ...
        public static  void UpdateExtension(string pathFileName,string extension)
        {
            if(!string.IsNullOrEmpty(pathFileName))
            {
                //初始化文件夹对象
                DirectoryInfo dir = new DirectoryInfo(pathFileName);
                File.Move(pathFileName, Path.ChangeExtension(pathFileName, extension));
           /*     FileInfo[] fileInfos = dir.GetFiles("*.*", SearchOption.TopDirectoryOnly);
                for (int i = 0; i < fileInfos.Length; i++)
                {
                    fileInfos[i].MoveTo(Path.ChangeExtension(fileInfos[i].FullName, extension));
                }*/
            }
        }
        ...
        /// <param name="DirPath">路径（含文件名）</param>
        /// <param name="Note">注释</param>
        /// <returns></returns>
        public bool CreateXmlDocument(string ProjectRoot,string RootName,string DirPath,string Note,string GUID)
        {
            try
            {
                xDocument = new XDocument(
                    new XElement(ProjectRoot, new XAttribute("ProjectName", RootName), new XAttribute("Note", Note), new XAttribute("GUID", GUID),
                       new XElement("DeviceConfig"),
                       new XElement("ChanConfig"),
                       new XElement("ViewConfig"),
                       new XElement("RecordConfig"))
                    );
                xDocument.Save(DirPath);
                return true;
            }
            catch (Exception e)
            {
        ...
        /// <returns></returns>
        public bool CreateXmlDocument(string FileName, string RootName, string Encode,string DirPath)
        {
            try
            {
